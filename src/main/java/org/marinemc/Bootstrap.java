...
public class Bootstrap {
	...
	static {
		if (System.getSecurityManager() == null)
			System.setSecurityManager(new MarineSecurityManager(System
					.getSecurityManager()));
	...
	public static void main(final String[] args) {
		...
		if (SystemUtils.getJavaVersion() < 7) {
			printf("-- Could not start MarineStandalone: Requires java {0} or above, you have {1} --",
					7, SystemUtils.getJavaVersion());
			System.exit(1);
		}
		 ...
		 * System.out.println(
		 * "-- Could not start MarineStandalone: Cannot run from compressed folder"
		 * ); System.exit(1); }
		 */
	...
	private void systemChecks() {
		if (SystemUtils.getJavaVersion() < 7) {
			System.out
					.println("-- Cannot Start The Server: Required Java 1.7 or above");
			System.exit(1);
		}
		if (SystemUtils.getArch() != 64)
			if (SystemUtils.getArch() == 32) {
				Logging.getLogger()
						.warn("Server is running on 32bit, this is not recommended as it can cause fatal errors and lag");
				Logging.getLogger().warn(
						"Consider updating java, or your hardware");
			} else
				Logging.getLogger()
						.warn("Unable to retrieve computer arch, either you're running a 128bit computer (impressive) or it's blocked.");
		final String args = StringUtils.join(ManagementFactory
				.getRuntimeMXBean().getInputArguments(), "");
		if (!args.contains("-Xmx"))
			Logging.getLogger()
					.warn("You haven't changed the JVM Memory Allocation Yet - It is recommended to increase the available memory through \"-Xmx[amount of ram]M\"!");
	}
	...
	private void start(final String[] args) {
		...
		final StartSettings settings = new StartSettings();
		// Start settings
		...
		Logging.getLogger()
				.logf("Starting MarineStandalone Server - Protocol Version §c§o{0}§0 (Minecraft §c§o{1}§0)",
		...
		if (ServerSettings.getInstance().offlineMode) {
			Logging.getLogger().warn(
					"-- You are running the server with §c§loffline§f mode");
			Logging.getLogger()
					.warn("-- Whilst this will allow you to play without internet access,");
			Logging.getLogger().warn(
					"-- it also gives hackers easy access to your server");
		}
		try {
			new Server(settings);
		} catch (final Throwable e) {
			Logging.getLogger().error(
					"Could not start the server, errors occurred", e);
		}
	}
