...
/**
 * A custom SecurityManager created for MarineStandalone. Will block pesky
 * actions made by plugins.
 *
 * @author Citymonstret
 */
public class MarineSecurityManager extends SecurityManager {
	 ...
	 * <p/>
	 * Used to indicate internal classes that shouldn't be used by plugins - or
	 * alike
	 */
	public static Permission MARINE_PERMISSION = new RuntimePermission(
			"marineInternal");
	 ...
	 *            Default security manager (can be null)
	 */
	 ...
	 *            Message to send
	 * @param allowOther
	 *            Allow loaders other than the plugin one
	 */
	private void status(final String msg, final boolean allowOther) {
		final Class[] context = getClassContext();
		ClassLoader loader;
		for (int i = 1; i < context.length; ++i)
			if ((loader = context[i].getClassLoader()) != null
					&& loader != ClassLoader.getSystemClassLoader())
				if (loader instanceof PluginClassLoader) {
					final Plugin plugin = ((PluginClassLoader) loader).plugin;
					Logging.getLogger().warn(
							"Plugin (" + plugin.getName()
									+ ") tried to use illegal methods: " + msg);
					throw new PluginSecurityException(plugin, msg);
				} else if (!allowOther)
					throw new AccessControlException(
							"Internal (Marine) Security Breach: " + msg);
	}
	 ...
	 */
	protected void checkInternal(final Permission permission) {
	...
	public static class PluginSecurityException extends AccessControlException {
		 ...
		 *
		 * @param plugin
		 *            Plugin that breached the security
		 * @param message
		 *            Message (Description)
		 */
		public PluginSecurityException(final Plugin plugin, final String message) {
			super("Plugin Security Breach: " + plugin.getName()
					+ " did an illegal action: " + message);
		}
