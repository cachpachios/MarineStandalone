...
public class Block implements Comparable<BlockID> {
	...
	public Block(final Chunk c, final int localX, final int localY,
			final int localZ) {
	...
	public Block(final Chunk c, final Position pos, final BlockID type) {
		this(c, pos.getX() / c.getPos().getX(), pos.getY(), pos.getZ()
				/ c.getPos().getY());
	}
	...
	public Position getGlobalPos() {
		if (referenceCheck())
			return null;
		return new Position(localX * chunk.get().getPos().getX(), y, localZ
				* chunk.get().getPos().getY());
	}

	public short getPacketID() {
		if (referenceCheck())
			return (short) 0;
		return (short) (getType().getIntID() << 4 & 0xfff0 | getType()
				.getMetaBlock());
	}

	@Override
	public int compareTo(final BlockID o) {
