...
public class PluginLoader {
	 ...
	 * @param desc
	 *            File to be checked
	 * @throws org.marinemc.plugins.PluginHandlerException
	 *             If the file uses a bad name
	 */
	public void checkIllegal(final PluginFile desc) {
		...
		for (final String blocked : BLOCKED_NAMES)
			if (main.toLowerCase().contains(blocked))
				throw new PluginHandlerException(this, "Plugin " + desc.name
						+ " contains illegal main class path");
	}
	...
	public void loadAllPlugins(final File folder) throws PluginHandlerException {
		if (!folder.exists() || !folder.isDirectory())
			throw new PluginHandlerException(this,
					"Invalid plugin folder (doesn't exist)");
				...
				if (new File(loader.getData(), "lib").exists()) {
					...
					for (final File f : fs)
						try {
							...
							new PluginHandlerException(this,
									"Could not load in lib " + f.getName(), e)
									.printStackTrace();
						}
				}
			} catch (final PluginHandlerException e) {
				Logging.getLogger().log(
						"Could not load in plugin: " + file.getName());
				e.printStackTrace();
			}
		}
	}
	...
	public void disableAllPlugins() {
		for (final Plugin plugin : manager.getPlugins())
			try {
				...
				new PluginHandlerException(this, "Could not properly disable "
						+ plugin.getName(), e).printStackTrace();
			}
	}
	 ...
	 * @throws PluginHandlerException
	 *             If the PluginClassLoader is unable to be loaded
	 */
	public PluginClassLoader loadPlugin(final File file)
			throws PluginHandlerException {
		if (!file.exists())
			throw new PluginHandlerException(this,
		...
		if (!data.exists())
			if (!data.mkdir())
				Logging.getLogger().warn(
						"Could not create data folder for " + desc.name);
		...
		try {
			...
			throw new PluginHandlerException(this,
					"Could not get the PluginClassLoader", e);
		}
	 ...
	 *             If the plugin is already disabled
	 */
	 ...
	 *
	 * @param file
	 *            Jar File, which contains the files you want to copy
	 * @param destination
	 *            Destination Folder
	 * @throws PluginHandlerException
	 *             If jar file cannot be loaded
	 */
	private void copyConfigIfExists(final File file, final File destination)
			throws PluginHandlerException {
		JarFile jar;
		try {
			jar = new JarFile(file);
		} catch (final IOException ioe) {
			throw new PluginHandlerException(this, "Could not load in "
					+ file.getName(), ioe);
		}
		...
		for (final JarEntry e : entryList)
			if (!e.getName().endsWith(".jar")) {
				...
				try {
					...
					new PluginHandlerException(this, "Could not copy jar file",
							exz).printStackTrace();
				}
			}
	}
	 ...
	 *
	 * @param file
	 *            File, should be a jarfile containing a "desc.json" and the
	 *            source folder
	 ...
	 * @throws PluginHandlerException
	 *             If a file cannot be loaded from the stream
	 */
	private PluginFile getPluginFile(final File file)
			throws PluginHandlerException {
		JarFile jar;
		try {
			jar = new JarFile(file);
		...
		if (desc == null)
			throw new PluginHandlerException(this,
					"Could not find desc.json in file: " + file.getName());
		InputStream stream;
		try {
			stream = jar.getInputStream(desc);
		} catch (final IOException ioe) {
			throw new PluginHandlerException(this,
					"Could not get stream for desc.json", ioe);
		}
		PluginFile f;
		try {
			...
			throw new PluginHandlerException(this,
					"Could not load in plugin file from stream", e);
		}
		try {
			jar.close();
		} catch (final Exception e) {
	...
	public boolean isTaken(String provider) {
		for (Plugin plugin : getManager().getPlugins()) {
			if (plugin.getProviderName().equals(provider)) {
				return true;
			}
		}
		return false;
